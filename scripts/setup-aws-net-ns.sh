...
AWS_NETWORK_INTERFACE=ens5

echo "[ ] Reading local network interface from .env"
source .env
echo "LOCAL_NETWORK_INTERFACE=$AWS_NETWORK_INTERFACE"
...
# Masquerading (i.e. IP translation)
iptables -t nat -A POSTROUTING -s 192.168.0.2/24 -o "$AWS_NETWORK_INTERFACE" -j MASQUERADE
